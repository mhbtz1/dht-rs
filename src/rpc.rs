use std::{io::{BufReader, BufWriter, Read, Write}, path::PathBuf};
...
impl <T: std::io::Write> RPCMessageEncoder<T> {
    ...
    fn metadata(&mut self, kind: u8, term: u64) {
        self.written_to_disk.extend_from_slice(&self.request_id.to_bytes_le());
    ...
    fn data(&mut self, data: &[u8]) {
        ...
        self.writer.write_all(&self.written_to_disk[offset..]).unwrap();
    }
...
impl RequestVoteReq {
    ...
    fn decode<T: std::io::Read> (
        ...
        let checksum = u32::from_le_bytes(buf[65..69].try_into().unwrap());
        if checksum != crc32c::crc32c(&buf[0..65]) {
        ...
        let last_log_term = u64::from_le_bytes(buf[57..65].try_into().unwrap());
...
impl AppendEntriesReq {
    fn send_rpc(&self, node_id: u64) -> anyhow::Result<()> {
        ...
        rpc_encoder.data(&self.entries.convert_to_bytes());
